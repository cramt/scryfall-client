...
impl Client {
    ...
    pub async fn name_in_set<S1: ToString, S2: ToString>(&self, name: S1, exact: bool, set: S2) -> Result<Card, ClientError> {
        let name = name.to_string();
        let set = set.to_string();
        let exact_or_fuzzy = if exact { "exact" } else { "fuzzy" };
        let query = self.named_client().form(&[(exact_or_fuzzy, name), ("set", set)]);
