...
impl UnpopulatedDecklist {
    pub async fn populate(&self) -> Result<PopulatedDecklist, ClientError> {
        ...
        let result = Client
            .collection_by_names(map.keys().map(|x| x.clone()).collect())
            .await?;
        let (main, sideboard) =
            result
                .into_iter()
                .fold((vec![], vec![]), |(mut main, mut sideboard), x| {
                    let possible_names = x
                        .name
                        .split(" // ")
                        .chain(vec![x.name.as_str()].into_iter());
        ...
        Ok(PopulatedDecklist { main, sideboard })
    }

    pub fn to_raw(&self) -> String {
        fn join(v: &Vec<(u16, String)>) -> String {
            v.into_iter()
                .map(|(amount, name)| format!("{} {}", amount, name).to_string())
                .collect::<Vec<String>>()
                .join("\r\n")
        }
        format!(
